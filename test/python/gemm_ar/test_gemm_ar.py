...
def perf_torch(
    ...
    for i in range(total_iters):
        ...
        torch.distributed.all_reduce(full_output, group=TP_GROUP, async_op=True)
        end_events[i].record()
...
def perf_flux(
    ...
    for i in range(total_iters):
        end_events[i].synchronize()
        if i >= warmup_iters:
            gemm_ar_times.append(start_events[i].elapsed_time(end_events[i]) / 1000)
    gemm_ar_time = sum(gemm_ar_times)

    gemm_time_ms = gemm_time / iters * 1000
    comm_time_ms = (gemm_ar_time - gemm_time) / iters * 1000
