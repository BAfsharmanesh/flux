...
struct ReduceScatterParams {
  ...
  bool use_p2p_read;
  void *args_workspace;
  void *scatter_ptr_aux[kMaxWorldSize] = {nullptr};
  void *barrier_ptr[kMaxWorldSize] = {nullptr};
  void *reduce_ptr[kMaxWorldSize] = {nullptr};
};
...
// TODO(houqi.1993) make sure aligned
template <typename T>
CUTLASS_DEVICE void
...
class ReduceScatterRing2dPull : public ReduceScatterBase<T, kM, kN, kFlattenTile> {
  ...
  static constexpr int kNumWorkersPerGroup = kWarpSize * 8;
  static constexpr int kNumThreadsPerGroup = kNumWorkersPerGroup + kWarpSize;
 ...
 public:
  ...
  run() {
    ...
    constexpr int kNumThreads = kNumWorkersPerGroup;
    static_assert(kNumThreads % kThreadsCol == 0, "");
    constexpr int kThreadRows = kNumThreads / kThreadsCol;
    auto thread_shape = cute::make_shape(cute::C<kThreadsCol>{}, cute::C<kThreadRows>{});
    auto thread_map_shape = cute::make_shape(
        cute::C<kVecLen>{},
        cute::C<kThreadsCol>{},
        cute::C<kThreadRows>{},
        cute::C<kTileSizeVec / kNumThreads>{});
    auto [tcol, trow] = cute::idx2crd((int)tid, thread_shape);

    auto set_ready_dev = [&](int *ptr) {
      sub_barrier_sync(gid_local, kNumThreadsPerGroup);
      if (tid == 0 || tid == kNumWorkersPerGroup) {
        atomic_store_release_dev(ptr, 1);
      }
    };
    auto wait_eq_sys = [&](int *ptr) {
      if (tid == 0) {
    ...
    for (int s = 0; s < kNumaWorldSize; s++) {
      ...
      for (auto sid : {NextNodeRank(segment), segment}) {
        ...
        for (int bid = gid; bid < tile_m_per_seg * tiled_n; bid += num_groups) {
          ...
          if (is_worker) {
            if (do_wait() && !skip_tile) {
              if (!is_inter_node) {  // wait for self gemm done
                ...
                print_worker("local [%d] = %d vs 1\n", tile_idx, *lock_ptr);
                wait_eq_dev(lock_ptr);
              }
              if (is_prev_inter_node) {
                int *lock_ptr = flags(rrank).epilogue_ptr(tile_idx);
                print_worker("remote epilogue %d[%d] = %d vs 1\n", rrank, tile_idx, *lock_ptr);
                wait_eq_sys(lock_ptr);
              } else {
            ...
            if (!is_inter_node) {  // no need for inter_node
              ...
              set_ready_dev(lock_ptr);
              print_waiter("set_ready: barrier[%d]=%d\n", reduce_tile_idx, *lock_ptr);
            }
          }
        }
        if (is_inter_node) {
...
class ReduceScatterRing2dPullGemmk : public ReduceScatterBase<T, kM, kN, kFlattenTile> {
  ...
  static constexpr int kWarpSize = 32;
  static constexpr int kNumWorkersPerGroup = kWarpSize * 4;
  static constexpr int kNumThreadsPerGroup = kNumWorkersPerGroup + kWarpSize;
 ...
 public:
  ...
  run() {
    ...
    auto set_ready = [&](int *ptr) {  // operate local flag. so use dev
      sub_barrier_sync(gid_local, kNumThreadsPerGroup);
      if (tid == 0 || tid == kNumWorkersPerGroup) {
        atomic_store_release_dev(ptr, 1);
      }
...
class ReduceScatterRing2dPushGemmk : public ReduceScatterBase<T, kM, kN, kFlattenTile> {
 ...
 public:
  ...
  run() {
    ...
    auto get_from_rank = [&](int rank) {
      // 0 -> 1 -> 2 -> 3 -> 7 -> 6 -> 5 -> 4
      int node = rank / params.sub_world_size;
      if (node == 0) {
        return (rank - 1 + params.sub_world_size) % params.sub_world_size;
      } else {
        return (rank + 1) % params.sub_world_size + params.sub_world_size;
      }
    };
    auto get_to_rank = [&](int rank) {
      int node = rank / params.sub_world_size;
      if (node == 0) {
        return (rank + 1) % params.sub_world_size;
      } else {
        return (rank - 1 + params.sub_world_size) % params.sub_world_size + params.sub_world_size;
      }
    };
    ...
    for (int s = 0; s < kNumaWorldSize; s++) {
      ...
      int from_rank = topo.rank_from[s][rank];
      ...
      for (auto sid : {NextNodeRank(segment), segment}) {
        ...
        for (int bid = gid; bid < tiles_per_rank; bid += num_groups) {
          ...
          if (is_worker) {
            if (!is_ring_start) {
              int *flag_ptr = flags(from_rank).reduce_ptr(tile_idx);
              print_worker("[%d:%d] wait for reduce[%d] = %d\n", rank, gid, tile_idx, *flag_ptr);
              wait_eq_sys(flag_ptr);
            }
            wait_eq_dev(flags(rank).epilogue_ptr(tile_idx));
            int offset = offset_per_seg * sid;
            int offset_mfree = offset_per_seg * topo.unused_segments_push[to_rank];
            auto rdata = reduce_buffer_tile(
                to_rank,
                is_inter_node ? m_free : m,
                n,
                tcol,
                trow,
                thread_map_shape,
                is_inter_node ? offset_mfree : offset);
            auto ldata = data_tile(rank, m, n, tcol, trow, thread_map_shape, offset);
            auto coord_v = coord(m, n, tcol, trow, thread_map_shape);
            int m_end = tiled_m_start * kM + m_per_rank;
            if (is_ring_start) {  // copy only
              ...
              auto local_reduce_buffer =
                  reduce_buffer_tile(rank, m, n, tcol, trow, thread_map_shape, offset);
              CUTLASS_PRAGMA_UNROLL
              for (int i = 0; i < size(ldata); i++) {
                if (cute::get<1>(coord_v(i)) < params.n && (cute::get<0>(coord_v(i)) < m_end)) {
            ...
            int *flag_ptr = is_inter_node ? flags(rank).reduce_sub_node_ptr(to_tile_idx(m0, n))
                                          : flags(rank).reduce_ptr(tile_idx);
            set_ready(flag_ptr);
            print_waiter("[%d:%d] set_ready %d[%d]=%d\n", rank, gid, to_rank, tile_idx, *flag_ptr);
          }
        }
        if (is_inter_node) {
          break;
    ...
    int m_end = tiled_m_start * kM + m_per_rank;
    int offset = offset_per_seg * rank;
    int offset_free = offset_per_seg * topo.unused_segments_push[rank];
    // reduce local
    for (int bid = gid; bid < tiles_per_rank; bid += num_groups) {
      int m = bid % tiled_m_per_rank, n = bid / tiled_m_per_rank;
      int m0 = m;
      ...
      if (is_worker) {
        ...
        wait_eq_dev(flag_ptr);
        flag_ptr = flags(get_from_rank(rank)).reduce_ptr(tile_idx);
        wait_eq_sys(flag_ptr);
        flag_ptr = flags((rank + params.sub_world_size) % params.world_size)
                       .reduce_sub_node_ptr(to_tile_idx(m0, n));
        wait_eq_sys(flag_ptr);
        auto ldata = data_tile(rank, m, n, tcol, trow, thread_map_shape, offset);
        auto rdata = reduce_buffer_tile(rank, m, n, tcol, trow, thread_map_shape, offset);
        auto rdata_next_numa =
            reduce_buffer_tile(rank, m_free, n, tcol, trow, thread_map_shape, offset_free);
        auto coord_v = coord(m, n, tcol, trow, thread_map_shape);
        CUTLASS_PRAGMA_UNROLL
        for (int i = 0; i < size(ldata); i++) {
          if (cute::get<1>(coord_v(i)) < params.n && (cute::get<0>(coord_v(i)) < m_end)) {
...
template <typename T, int kM, int kN, bool kFlattenTile>
class ReduceScatterRing2dPushGemmkSharedWaiter
    : public ReduceScatterBase<T, kM, kN, kFlattenTile> {
  USE_REDUCE_SCATTER_BASE;
  static constexpr int kWarpSize = 32;
  static constexpr int kNumWorkersPerGroup = kWarpSize * 2;

 public:
  CUTLASS_DEVICE
  ReduceScatterRing2dPushGemmkSharedWaiter(const ReduceScatterParams &param) : Base(param) {}

  CUTLASS_DEVICE void
  run() {
    int gid_local = threadIdx.x / kNumWorkersPerGroup;
    int num_groups_per_cta = (blockDim.x - kWarpSize) / kNumWorkersPerGroup;  // 15
    int num_threads_per_cta = kNumWorkersPerGroup * num_groups_per_cta + kWarpSize;
    if (threadIdx.x >= num_threads_per_cta)
      return;
    bool is_worker = gid_local < num_groups_per_cta;
    int gid_this_group = num_groups_per_cta * blockIdx.x;
    int gid = gid_local + gid_this_group;
    int num_groups = num_groups_per_cta * gridDim.x;
    int tid = threadIdx.x % kNumWorkersPerGroup;

    auto set_ready = [&](int *ptr) {
      sub_barrier_sync(num_groups_per_cta, num_threads_per_cta);
      if (tid == 0) {
        atomic_store_release_sys(ptr, 1);
      }
    };
    auto wait_eq_sys = [&](int *ptr) {
      ...
      sub_barrier_sync(gid_local, kNumWorkersPerGroup);
    };
    auto wait_eq_dev = [&](int *ptr) {
      if (tid == 0) {
        atomic_ref_dev<int> ref(*ptr);
    ...
    for (int s = 0; s < kNumaWorldSize; s++) {
      int to_rank = topo.rank_to[s][rank];
      int from_rank = topo.rank_from[s][rank];
      bool is_inter_node = lnode != (to_rank / kNumaWorldSize);
      bool is_ring_start = ((topo.rank_from[s][rank]) / kNumaWorldSize) != lnode;
      int segment = topo.segments[s][lnode];
      for (auto sid : {NextNodeRank(segment), segment}) {
        int tiled_m_start = tiled_m_per_rank * sid;  // open set
        for (int bid = gid, bid_wave_start = gid_this_group; bid_wave_start < tiles_per_rank;
             bid += num_groups, bid_wave_start += num_groups) {
          int m = bid % tiled_m_per_rank, n = bid / tiled_m_per_rank;
          int m_start = bid_wave_start % tiled_m_per_rank,
              n_start = bid_wave_start / tiled_m_per_rank;
          int tile_idx_start = to_tile_idx(m_start + tiled_m_start, n_start);

          if (is_worker) {
            if (bid < tiles_per_rank) {
              int m_free = m + topo.unused_segments_push[to_rank] * tiled_m_per_rank;
              m += tiled_m_start;
              if (!is_ring_start) {
                int *flag_ptr = flags(from_rank).reduce_ptr(tile_idx_start);
                print_worker(
                    "[%d:%d] wait for reduce[%d] = %d\n", rank, gid, tile_idx_start, *flag_ptr);
                wait_eq_sys(flag_ptr);
              }
              wait_eq_dev(flags(rank).epilogue_ptr(to_tile_idx(m, n)));
              int offset = offset_per_seg * sid;
              int offset_mfree = offset_per_seg * topo.unused_segments_push[to_rank];
              auto rdata = reduce_buffer_tile(
                  to_rank,
            ...
            int *flag_ptr = is_inter_node
                                ? flags(rank).reduce_sub_node_ptr(to_tile_idx(m_start, n_start))
                                : flags(rank).reduce_ptr(tile_idx_start);
            set_ready(flag_ptr);
            print_waiter(
                "[%d:%d] set_ready %d[%d]=%d\n", rank, gid, to_rank, tile_idx_start, *flag_ptr);
          }
        }
        if (is_inter_node) {
          break;
    ...
    int offset = offset_per_seg * rank;
    int offset_free = offset_per_seg * topo.unused_segments_push[rank];
    int rank_next_node = (rank + params.sub_world_size) % params.world_size;
    // reduce local
    for (int bid = gid, bid_wave_start = gid_this_group; bid_wave_start < tiles_per_rank;
         bid += num_groups, bid_wave_start += num_groups) {
      if (is_worker && bid < tiles_per_rank) {
        int m = bid % tiled_m_per_rank, n = bid / tiled_m_per_rank;
        int m_start = bid_wave_start % tiled_m_per_rank,
            n_start = bid_wave_start / tiled_m_per_rank;
        int m0 = m;
        ...
        int *flag_ptr = flags(rank).epilogue_ptr(tile_idx);
        wait_eq_dev(flag_ptr);
        flag_ptr = flags(get_from_rank(rank)).reduce_ptr(tile_idx_start);
        wait_eq_sys(flag_ptr);
        flag_ptr = flags(rank_next_node).reduce_sub_node_ptr(to_tile_idx(m_start, n_start));
        wait_eq_sys(flag_ptr);
        auto ldata = data_tile(rank, m, n, tcol, trow, thread_map_shape, offset);
        auto rdata = reduce_buffer_tile(rank, m, n, tcol, trow, thread_map_shape, offset);
        auto rdata_next_numa =
  ...
  static void
  invoke(const ReduceScatterParams &params, SharedStorage &) {
    ReduceScatterRing2dPushGemmkSharedWaiter<T, kM, kN, kFlattenTile> op(params);
    op.run();
  }
};
    ...
    for (int s = 0; s < kNumaWorldSize; s++) {
      ...
      for (int segment : {NextNodeRank(topo.segments[s][rnode]), topo.segments[s][rnode]}) {
        ...
        for (int i = blockIdx.x; i < tiles_per_rank; i += gridDim.x) {
          ...
          print_per_block("set_ready: barrier[%d]=%d\n", tile_idx, *barrier_ptr);
        }
        if (is_inter_node) {
...
class ReduceScatterRing1dPullGemmk : public ReduceScatterBase<T, kM, kN, kFlattenTile> {
 ...
 public:
  ...
  run() {
    ...
    constexpr int kNumWorkersPerGroup = kWarpSize * 2;

    int gid_local = threadIdx.x / kNumWorkersPerGroup;
    int num_groups_per_cta = (blockDim.x - kWarpSize) / kNumWorkersPerGroup;  // 15
    if (threadIdx.x >= num_groups_per_cta * kNumWorkersPerGroup + kWarpSize)
      return;
    bool is_worker = gid_local < num_groups_per_cta;
    int gid = is_worker ? (gid_local + num_groups_per_cta * blockIdx.x)
                        : num_groups_per_cta * blockIdx.x;  // for wait flags
    int num_groups = num_groups_per_cta * gridDim.x;
    int tid = threadIdx.x % kNumWorkersPerGroup;
    constexpr int kNumThreads = kNumWorkersPerGroup;
    // force divided by caller: no assert for kernel
    constexpr int kThreadRows = (kNumThreads + kThreadsCol - 1) / kThreadsCol;
    auto thread_shape = cute::make_shape(cute::C<kThreadsCol>{}, cute::C<kThreadRows>{});
    auto thread_map_shape = cute::make_shape(
        cute::C<kVecLen>{},
        cute::C<kThreadsCol>{},
    ...
    auto set_ready = [&](int *ptr, int groups) {
      sub_barrier_sync(num_groups_per_cta, kNumWorkersPerGroup * groups + kWarpSize);
      if (tid == 0) {
        atomic_store_release_sys(ptr, 1);
      }
    };
    ...
    const int rrank = (rank + 1) % params.world_size;
    for (int i = 0; i < params.world_size - 1; i++) {
      int segment = (i + rrank + 1) % params.world_size;
      ...
      for (int bid = gid, bid_wave_start = blockIdx.x * num_groups_per_cta;
           bid_wave_start < tiles_per_rank;
           bid += num_groups, bid_wave_start += num_groups) {
        int m = bid % tiled_m_per_rank, n = bid / tiled_m_per_rank;
        int m_start = bid_wave_start % tiled_m_per_rank,
            n_start = bid_wave_start / tiled_m_per_rank;
        ...
        if (is_worker) {
          if (bid < tiles_per_rank) {
            wait_eq_dev(flags(rank).epilogue_ptr(tile_idx));
            if (i != 0) {
              print_worker("wait reduce tile %d", tile_idx_start);
              wait_eq(flags(rrank).reduce_ptr(tile_idx_start));
            } else {
              print_worker("wait gemm tile %d", tile_idx);
              wait_eq(flags(rrank).epilogue_ptr(tile_idx));
            }
            auto ldata = data_tile(rank, m, n, tcol, trow, thread_map_shape, offset);
            auto rdata = data_tile(rrank, m, n, tcol, trow, thread_map_shape, offset);
            auto coord_v = coord(m, n, tcol, trow, thread_map_shape);
          ...
          sub_barrier_sync(
              num_groups_per_cta, kNumWorkersPerGroup * num_groups_per_cta + kWarpSize);
        } else {
          int *flag_ptr = flags(rank).reduce_ptr(tile_idx_start);
          set_ready(flag_ptr, num_groups_per_cta);
          print_waiter(
              "%d:%d ready at reduce %d[%d] to %d\n", rank, gid, rrank, tile_idx_start, *flag_ptr);
        }
      }
    }
        ...
        if (is_worker) {  // doing copy here
          ...
          if (bid < tiles_per_rank) {
            ...
            if (!is_ring_start) {  // also wait for reduce buffer ready to do reduce
              int *flag_ptr = flags(rank).reduce_ptr(tile_idx_start);
              print_worker("%d:%d wait reduce %d to be 1\n", rank, gid, tile_idx_start);
              wait_eq(flag_ptr);
            }

            auto ldata = data_tile(rank, m, n, tcol, trow, thread_map_shape, offset);
            auto lrdata = reduce_buffer_tile(rank, m, n, tcol, trow, thread_map_shape, offset);
            auto rdata = reduce_buffer_tile(rrank, m, n, tcol, trow, thread_map_shape, offset);

            int start = m * kM + trow;
            int end = min(start + kM, m_end);
            int copy_counts = (end - start + kThreadRows - 1) / kThreadRows;  // size(ldata)
          ...
          int *flag_ptr = flags(rrank).reduce_ptr(tile_idx_start);
          set_ready(flag_ptr, num_groups_per_cta);
          print_waiter(
              "%d:%d set_ready at reduce %d[%d] to %d\n",
              rank,
              gid,
              rrank,
              tile_idx_start,
...
#ifndef FLUX_REDUCE_SCATTER_WITH_NCCL  // don't use this. too slow. don't know why
...
class ReduceScatterOp : public ReduceScatterOpBase<T> {
 ...
 public:
  ...
  initialize(const Arguments &args) {
    static int duration_ns = get_int_from_env("FLUX_RS_SLEEP_NS", 100000);
    ...
    for (int i = 0; i < params_.world_size; i++) {
      params_.reduce_ptr[i] = args.reduce_buffer_ptrs[i];
